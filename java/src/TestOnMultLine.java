...
public class TestOnMultLine {

    public static void onMultLine(int m_ar, int m_br, FileWriter writer) throws IOException {
        long Time1, Time2;
        String st;
        double temp;
        int i, j, k;
    ...
    public static void main(String[] args) {
        ...
        try {
            File file = new File("single_mult_line_java.csv");
            writer = new FileWriter(file);
            writer.write("size,time\n"); // Column headers
            for (int i = 600; i <= 3000; i += 400) {
                for (int j = 0; j < 2; j++) {
                    onMultLine(i, i, writer);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
