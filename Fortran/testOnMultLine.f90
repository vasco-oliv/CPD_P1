subroutine OnMultLine(m_ar, m_br)
    ...
    do i = 1, m_ar
        do j = 1, m_br
            temp = 0.0d0
            do k = 1, m_ar
                temp = pha(i, j) * phb(j,k)
                phc(i, k) = phc(i, k) + temp
            end do
        end do
    end do
    ...
    open(unit=unit, file='single_mult_line_fortran.csv', status='old', action='write', position='append', iostat=ierr)
    if (ierr /= 0) then
